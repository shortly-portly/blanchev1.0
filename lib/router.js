...
Router.route('/', {
  ...
  data: function() {
    var wibble = Reviews.find({user: Meteor.userId()});
    return {myData: Reviews.find({user: Meteor.userId()})};
  },
  action: function() {
    if (this.ready()) {
      this.render("home");
...
Router.route('/editPerformance/:_id', {
  ...
  data: function() {
    if (this.ready() ) {
      ...
      review = Reviews.findOne({_id: this.params._id});

      return {questions: Questions.find({appraisal: review.appraisal}),
              review: Reviews.findOne({_id: this.params._id})};
  }},
  action: function() {
...
Router.route('/newAppraisalForm', function() {
...
Router.route('/editAppraisalForm/:_id', {
  ...
  data: function() {
    ...
    return {questions: Questions.find({appraisal: this.params._id}),
            appraisalForm: Appraisals.findOne(this.params._id)};
  },
  action: function() {
    if (this.ready()) {
      this.render('editAppraisalForm');
    }
  }
});
...
Router.route('/listAppraisals', {
  ...
  data: function() {
    return {appraisals: Appraisals.find()};
  },
  action: function() {
    if (this.ready()) {
